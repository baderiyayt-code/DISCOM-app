...
const i18n = {
    ...
    hi: {
        ...
        toastSettings: "सेटिंग्स सेव हो गईं!", toastDel: "सफलतापूर्वक डिली মিলিটারি हुआ!", toastImport: "सफलतापूर्वक इम्पोर्ट हुआ!",
...
window.syncToSupabase = async function(manual = false) {
    ...
    try {
        ...
        Object.values(appState.gssNodes).forEach(g => {
            if(g && g.code && checkDirty('GSS', g.code)) { gssPayload.push({ gss_code: String(g.code), gss_name: g.name, lat: g.lat, lng: g.lng, user_id: uid }); }
        });

        Object.keys(appState.feeders).forEach(fCode => {
            const net = appState.feeders[fCode];
            if(net && net.feeder && net.feeder.code && checkDirty('FEEDER', fCode)) { fdrPayload.push({ feeder_code: String(fCode), gss_code: String(net.feeder.parentGss), feeder_name: net.feeder.name, user_id: uid }); }
    ...
    try {
        ...
        if (gssRes.data) {
            gssRes.data.forEach(g => {
                ...
                if (!localG || cloudTime >= localTime) {
                    appState.gssNodes[g.gss_code] = { code: g.gss_code, name: g.gss_name, lat: g.lat, lng: g.lng, updatedAt: cloudTime };
                }
            });
        }
        ...
        if (fdrRes.data) {
            fdrRes.data.forEach(f => {
                ...
                const cloudTime = f.updatedAt || 0;
                const localTime = localF && localF.feeder ? (localF.feeder.updatedAt || 0) : 0;
                
                if (!localF) {
                    appState.feeders[f.feeder_code] = { feeder: { code: f.feeder_code, name: f.feeder_name, parentGss: f.gss_code, subdivCode: "SD-01", updatedAt: cloudTime }, poles: [], dts: [], lines: [], consumers: [] };
                } else if (cloudTime >= localTime) {
                    appState.feeders[f.feeder_code].feeder = { code: f.feeder_code, name: f.feeder_name, parentGss: f.gss_code, subdivCode: "SD-01", updatedAt: cloudTime };
                }
            });
        }
        ...
        if (objRes.data) {
            objRes.data.forEach(obj => {
                ...
                let d = obj.data; if(!d) return;
                d.id = obj.id; d.feederCode = fc;
                const cloudTime = d.updatedAt || 0;

                const arrName = obj.type === 'POLE' ? 'poles' : obj.type === 'DT' ? 'dts' : obj.type === 'LINE' ? 'lines' : obj.type === 'CONSUMER' ? 'consumers' : null;
                if(!arrName) return;

                const localArr = appState.feeders[fc][arrName];
                const existingIdx = localArr.findIndex(x => x.id === d.id);

                if (existingIdx === -1) {
                    localArr.push(d); 
                } else {
                    const localObj = localArr[existingIdx];
                    const localTime = localObj.updatedAt || 0;
                    if (cloudTime > localTime) { 
                        if (localObj.photo && !d.hasPhoto && d.photo === undefined) d.photo = localObj.photo; 
                        localArr[existingIdx] = d; 
                    }
                }
            });
        }

        if (appState.deletedItems.gss && appState.deletedItems.gss.length > 0) appState.deletedItems.gss.forEach(id => delete appState.gssNodes[id]);
        if (appState.deletedItems.feeders && appState.deletedItems.feeders.length > 0) appState.deletedItems.feeders.forEach(id => delete appState.feeders[id]);
        if (appState.deletedItems.objects && appState.deletedItems.objects.length > 0) {
            const delIds = new Set(appState.deletedItems.objects);
            Object.values(appState.feeders).forEach(net => {
                if(net.poles) net.poles = net.poles.filter(x => !delIds.has(x.id));
                if(net.dts) net.dts = net.dts.filter(x => !delIds.has(x.id));
                if(net.lines) net.lines = net.lines.filter(x => !delIds.has(x.id));
                if(net.consumers) net.consumers = net.consumers.filter(x => !delIds.has(x.id));
            });
        }
